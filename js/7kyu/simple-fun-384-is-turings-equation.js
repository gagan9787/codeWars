...
function isTuringEquation(s){
  const num1 = s.split("+")[0].split("").reverse().join("");
  const num2 = s.split("+")[1].split("=")[0].split("").reverse().join("");
  const num3 = s.split("+")[1].split("=")[1].split("").reverse().join("");
  return Number(num1) + Number(num2) === Number(num3) ? true : false;
}
